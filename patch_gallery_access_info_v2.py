...
# 1. Drop ChevronDown (only used by the toggle we're removing)
old_icons = "import { Images, ChevronRight, ChevronDown, Search, SlidersHorizontal, Lock, Copy, Check } from 'lucide-react'"
assert src.count(old_icons) == 1, "icon import anchor not found or not unique"
new_icons = "import { Images, ChevronRight, Search, SlidersHorizontal, Lock, Copy, Check } from 'lucide-react'"
src = src.replace(old_icons, new_icons)

# 2. Replace AccessInfoRow + GalleryRow entirely: AccessInfoItem is now a
#    compact inline label+value+copy unit (each field copies independently
#    and shows its own "Copied" feedback -- not one shared copy button for
#    both values). GalleryRow drops the toggle/expand state -- the strip is
#    always visible when the gallery has protection, no click needed to
#    reveal it -- and the link now opens in a new tab so the portal (with
#    the codes still visible) stays open behind it instead of navigating
#    away.
old_block = '''function AccessInfoRow({ label, value }) {
...
assert src.count(old_block) == 1, "AccessInfoRow/GalleryRow anchor not found or not unique"

new_block = '''function AccessInfoItem({ label, value }) {
  const [copied, setCopied] = useState(false)
  ...
  return (
    <span className="flex items-center gap-1 text-xs" style={{ color: 'var(--text-muted)' }}>
      {label} <span className="font-mono font-medium" style={{ color: 'var(--text)' }}>{value}</span>
      <button type="button" onClick={handleCopy} aria-label={`Copy ${label.toLowerCase()}`}
        style={{ background: 'none', border: 'none', cursor: 'pointer', color: copied ? '#22c55e' : 'var(--text-muted)', display: 'flex', padding: 2 }}>
...
path.write_text(src)
print("Switched Access info to the always-visible inline strip with per-field copy, and gallery links now open in a new tab")
