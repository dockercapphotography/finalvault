-- Migration: 073_inquiry_notify_split_date_time.sql
-- The "New inquiry" photographer notification email crammed date and
-- time into one "Requested" row ("Sunday, September 20 around 1:00 PM
-- – 1:30 PM"), which now wraps onto two lines since 071 added the end
-- time. The "New booking" notification (claim_signup_slot) already
-- shows Date and Time as two separate rows -- this brings the inquiry
-- notification in line with that same layout: "Date" / "Time" rows,
-- same labels, same style, matching exactly.
--
-- Only change from 072_fix_stale_email_domain.sql's version of this
-- function: the notify_html "Requested" row is now two rows.
--
-- Run after: 072_fix_stale_email_domain.sql
