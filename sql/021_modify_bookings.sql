-- Migration: 021_modify_bookings.sql
-- Feature: Photographer-initiated booking changes -- moving an existing
-- claimed slot to a different open slot (optionally a different shoot
-- type, e.g. a client upgrading), and manually adjusting a claimed slot's
-- own time to something outside the pre-generated slot grid entirely.
--
-- Both RPCs are restricted to the owning photographer (checked via
-- auth.uid()), unlike claim_signup_slot which is intentionally callable
-- by an anonymous public visitor. Both rely on the existing
-- no_overlapping_claimed_slots EXCLUDE constraint (GiST, on
-- (signup_page_id, time_range) WHERE claimed_at IS NOT NULL) to enforce
-- "doesn't conflict with an existing booking" -- open slots are already
-- exempt from that constraint, so no separate conflict-checking query is
-- needed; the UPDATE itself is the check, same pattern claim_signup_slot
-- already uses (catch exclusion_violation).
--
-- Both keep the linked `sessions` row in sync (session_date/start_time/
-- end_time, recomputed in the venue's timezone the same way
-- claim_signup_slot originally computed them) and can optionally send the
-- client a "your appointment time has changed" email, reusing the same
-- branded template shape as the original booking confirmation, with an
-- ICS attachment using the SAME calendar UID (so calendar apps update the
-- existing event instead of creating a duplicate) and an incremented
-- SEQUENCE.
--
-- Run after: 020_fix_preferences_collision.sql
...
BEGIN
  ...
  -- Keep the linked session in sync -- date/time recomputed the same way
  -- claim_signup_slot originally computed them, and the session's own
  -- type/name updated too if this move crossed shoot types (a genuine
  -- service change, e.g. an upgrade), not just a time change.
  ...
  IF p_notify_client AND v_source.client_email IS NOT NULL THEN
    BEGIN
      ...
      -- Same UID as the original booking's ICS + SEQUENCE:1 so calendar
      -- apps treat this as an update to the existing event, not a new one.
  ...
  RETURN json_build_object('success', true, 'target_slot_id', p_target_slot_id);
END;
$function$;
...
BEGIN
  ...
  RETURN json_build_object('success', true, 'start_time', v_slot.start_time, 'end_time', v_slot.end_time);
END;
$function$;
