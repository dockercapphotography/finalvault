-- 046_microsite_hero_mosaic_images.sql
-- Mosaic previously reused whatever was picked for the Gallery section
-- automatically -- implicit, hidden coupling that made the editor feel
-- unpredictable (extra images with no visible control for them). Gives
-- Mosaic its own dedicated, explicitly-picked image set instead.
--
-- Run after: 045_microsite_hero_family.sql

ALTER TABLE microsites
  ADD COLUMN IF NOT EXISTS hero_mosaic_image_keys text[] DEFAULT '{}';
