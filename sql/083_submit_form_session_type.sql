-- 0XX_submit_form_session_type.sql
-- Rename to the next available sql/0NN_ number before running.
--
-- Adds `type` to get_submit_form_data's returned session object --
-- sessions.type already exists (used elsewhere, e.g. crmApi.js's
-- resolveTemplateVariables session_type var) but wasn't being selected
-- by this RPC. CREATE OR REPLACE on the same function signature, so this
-- is a pure addition, nothing else about the function changes.
