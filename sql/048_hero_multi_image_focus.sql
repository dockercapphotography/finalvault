-- 048_hero_multi_image_focus.sql
-- Adds per-image focal point support for Cycle and Mosaic, which each
-- use multiple images and so can't share a single hero_focus_x/y like
-- Single/Zoom do. Each is a jsonb object mapping { [r2_key]: {x, y} },
-- default '{}' -- any image without an entry falls back to centered
-- (0.5, 0.5) cropping, so this is fully backward compatible.
--
-- Run after: 047_microsite_dark_logo.sql

ALTER TABLE microsites
  ADD COLUMN IF NOT EXISTS hero_cycle_image_focus jsonb NOT NULL DEFAULT '{}',
  ADD COLUMN IF NOT EXISTS hero_mosaic_image_focus jsonb NOT NULL DEFAULT '{}';
