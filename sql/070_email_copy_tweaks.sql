-- Migration: 070_email_copy_tweaks.sql
-- Two small copy fixes, both from side-by-side comparison of the
-- "Inquiry received!" and "You're confirmed!" emails:
--
-- 1. submit_signup_inquiry's client email only ever displayed the
--    requested START time ("10:00 AM"), even though v_end_time was
--    already being computed internally for the window/conflict checks.
--    Now displays a range ("10:00 AM – 11:00 AM"), matching the format
--    update_session_status's confirmed email already uses.
--
-- 2. In both emails, "Here's what you requested:" / "Here are your
--    confirmed details:" was the tail of the same sentence as the "Hi
--    ___" greeting. Split into its own paragraph in both.
--
-- Full function bodies below are otherwise identical to the live
-- versions (069_session_status_confirmation_email.sql for
-- update_session_status; pre-existing for submit_signup_inquiry) --
-- only the two changes above are applied.
--
-- Run after: 069_session_status_confirmation_email.sql
    ...
    IF v_resend_key IS NOT NULL AND v_notify_email IS NOT NULL THEN
      v_notify_html := '<!DOCTYPE html><html><body style="margin:0;padding:0;background:#f4f4f5;font-family:-apple-system,BlinkMacSystemFont,''Segoe UI'',sans-serif;">' ||
        ...
        '<table cellpadding="0" cellspacing="0" width="100%" style="border:1px solid #e5e7eb;border-radius:12px;overflow:hidden;margin-bottom:20px;">' ||
          '<tr><td style="padding:10px 16px;border-bottom:1px solid #e5e7eb;width:70px;color:#9ca3af;font-size:12px;">Requested</td><td style="padding:10px 16px;border-bottom:1px solid #e5e7eb;color:#111111;font-size:13px;">' || v_local_date || ' around ' || v_local_time || '</td></tr>' ||
