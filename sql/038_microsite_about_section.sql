-- 038_microsite_about_section.sql
-- Adds a real About section: a dedicated photo (separate from the hero
-- image), a heading, and an editable stats list (label/value pairs, not
-- hardcoded to any specific metrics -- a photographer defines their own).
--
-- Run after: 037_microsite_logo_in_rpc.sql

ALTER TABLE microsites
  ADD COLUMN IF NOT EXISTS about_heading text,
  ADD COLUMN IF NOT EXISTS about_photo_key text,
  ADD COLUMN IF NOT EXISTS about_stats jsonb NOT NULL DEFAULT '[]'::jsonb;
  -- about_stats shape: [{ "value": "200+", "label": "Sessions Shot" }, ...]
