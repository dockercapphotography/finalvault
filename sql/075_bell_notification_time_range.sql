-- Migration: 075_bell_notification_time_range.sql
-- Extends the email time-range fixes (070-074) to the in-app bell
-- notifications, which had the same single-time gap:
--   1. notify_claim_push() -- the "X claimed a slot" bell for slot
--      bookings showed only the start time.
--   2. submit_signup_inquiry -- the "X sent an inquiry" bell showed
--      only the requested start time.
-- Both now show a range. Note these bodies are plain text (rendered
-- directly in NotificationBell.jsx, not HTML), so a literal unicode en
-- dash (–) is used here rather than the &ndash; HTML entity the email
-- templates use -- an entity would show up as literal text in the bell.
--
-- The actual push notification payload sent to the photographer's phone
-- (supabase/functions/send-claim-push/index.ts) has the same gap but is
-- a separate TypeScript file, not touched by this migration.
--
-- Run after: 074_fix_confirmed_email_subject.sql
    ...
    if v_photographer_id is not null then
      ...
      values (
        ...
        coalesce(v_shoot_type_name, 'Unknown shoot type') || ' · ' ||
          ...
          to_char(new.end_time at time zone coalesce(v_timezone, 'UTC'), 'FMHH12:MI AM'),
