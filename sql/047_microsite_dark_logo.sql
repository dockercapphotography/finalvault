-- 047_microsite_dark_logo.sql
-- Adds logo_dark_r2_key -- an optional second logo variant for use
-- against light backgrounds (scrolled nav on a light theme, footer on
-- a light theme). logo_r2_key remains the primary/light-background-
-- friendly logo, used everywhere the backdrop is dark. If
-- logo_dark_r2_key is never set, everything falls back to logo_r2_key
-- exactly as it does today -- fully backward compatible.
--
-- Run after: 046_microsite_hero_mosaic_images.sql

ALTER TABLE microsites
  ADD COLUMN IF NOT EXISTS logo_dark_r2_key text;
