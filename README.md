...
- **Flexible downloads** — three independent download types: gallery display (WebP preview), web-size JPEG (generated at upload time, served directly), and full-resolution original; both web-size and full-resolution ZIP downloads of large galleries (over 25 images or 250MB) process in the background with an email link when ready, from either the client-facing gallery or your own dashboard; a repeated request for the exact same set of photos reuses an already-prepared ZIP instead of rebuilding it
...
- **Questionnaire templates** — build reusable question sets (short/long text, yes/no, single/multi-choice), optionally require email/name/agreement to a terms checkbox; assign to a session or a Signup Page shoot type; optionally redirect people somewhere after they submit -- your Instagram, a review link, or any custom URL -- with an optional short auto-redirect countdown
...
- **Downloads** — web-size JPEG or full-resolution original, individual or full-gallery ZIP; large web-size or full-resolution ZIPs are prepared in the background with an email link when ready
