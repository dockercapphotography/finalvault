...
- **Client CRM** — create and manage client records with contact info, avatars, pronouns, tags, and linked galleries (a gallery can be linked to multiple clients, e.g. both spouses in a wedding, each with full portal access); Google Places address autocomplete; chip+typeahead tag input with autocomplete; search, tag filtering (multi-select), and Sort By (Name, Recently added)
