...
# ── 1. Fix duplicate — use getByTitle directly, not scoped to broad div ───────
old = '''      await expect(page.getByText(q.name).first()).toBeVisible({ timeout: 8000 })
      // Find the row by locating the text, then finding the sibling Duplicate button
...
# ── 3. Fix submission form test — questionnaire name not rendered on page ─────
...
with open(path, 'w') as f:
    f.write(src)
print("✅ Patched 3 remaining failing tests")
