...
export async function getClientGalleries(clientId) {
  const { data, error } = await supabase
    .from('galleries')
    .select('id, title, event_name, event_date, is_active, expires_at, share_token, created_at, cover_image_id, cover_r2_key, gallery_images!cover_image_id(preview_r2_key), gallery_clients!inner(client_id)')
